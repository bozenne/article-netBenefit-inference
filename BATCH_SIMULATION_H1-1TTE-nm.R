...
## setwd(path)
## source("BATCH_SIMULATION_H1-1TTE-nm.R")
## sbatch -a 1-10 -J 'mytest' --output=/dev/null --error=/dev/null R CMD BATCH --vanilla BATCH_SIMULATION_H1-1TTE-nm.R /dev/null 
...
## * path
path <- "."
path.res <- file.path(path,"Results","SIMULATION_H1-1TTE-nm")
...
path.output <- file.path(path,"output","SIMULATION_H1-1TTE-nm")
...
for(iN in 1:length(seqN)){ ## iN <- 7
    ...
    for(iSim in 1:n.sim){ ## iSim <- 1
        ...
        ## exp(coef(survival::coxph(survival::Surv(time, status) ~ treatment, data = dt)))
        ## exp(coef(survival::coxph(survival::Surv(timeU, One) ~ treatment, data = dt)))
        ...
        ## microbenchmark::microbenchmark(u = BuyseTest(ff.test, data = dt, scoring.rule = "Peron", method.inference = "u-statistic"),
        ## none = BuyseTest(ff.test, data = dt, scoring.rule = "Peron", method.inference = "none"),
        ## times = 1)
        ...
        iOutPeron1 <- confint(e.Peron)
            ...
            res <- rbind(res,
                         data.table(iOutGS1,
                                    ...
                                    endpoint = rownames(iOutGS1), threshold = threshold[iT], N=iN.T+iN.C, n.T = iN.T, n.C = iN.C, iter = iSim, Hprojection = 1, method = "GS"),
                         data.table(iOutGS2,
                                    timeAll = tps.GS["elapsed"], timeEstimate = tps.GS0["elapsed"],
                                    endpoint = rownames(iOutGS2), threshold = threshold[iT], N=iN.T+iN.C, n.T = iN.T, n.C = iN.C, iter = iSim, Hprojection = 2, method = "GS"),
                         data.table(iOutGehan1,
                                    timeAll = tps.Gehan["elapsed"], timeEstimate = tps.Gehan0["elapsed"],
                                    endpoint = rownames(iOutGehan1), threshold = threshold[iT], N=iN.T+iN.C, n.T = iN.T, n.C = iN.C, iter = iSim, Hprojection = 1, method = "Gehan"),
                         data.table(iOutGehan2,
                                    timeAll = tps.Gehan["elapsed"], timeEstimate = tps.Gehan0["elapsed"],
                                    endpoint = rownames(iOutGehan2), threshold = threshold[iT], N=iN.T+iN.C, n.T = iN.T, n.C = iN.C, iter = iSim, Hprojection = 2, method = "Gehan"),
                         data.table(iOutPeron1,
                                    timeAll = tps.Peron["elapsed"], timeEstimate = tps.Peron0["elapsed"],
                                    endpoint = rownames(iOutPeron1), threshold = threshold[iT], N=iN.T+iN.C, n.T = iN.T, n.C = iN.C, iter = iSim, Hprojection = 1, method = "Peron"))
        }
    }
    cat("\n")
